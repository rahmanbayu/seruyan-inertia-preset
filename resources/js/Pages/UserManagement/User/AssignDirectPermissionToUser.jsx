...
export default function AssignDirectPermissionToUser(props) {
    const { setter, getter, permissions } = props;
    ...
    useEffect(() => {
        setData('permissions', getter ? getter.permissions.map(i => i.id + '') : [])
    }, [getter]);
    ...
    function submit(e) {
        e.preventDefault()
        post(route('users.assign_direct_permission', getter), {
            onSuccess: () => {
                setter(undefined)
                clearErrors()
                reset()
            ...
            <div className={`${getter != undefined ? '' : 'translate-y-5 opacity-0 pointer-events-none'} transform transition-all duration-150 ease-in fixed inset-0 flex items-center justify-center z-20 pointer-events-none backdrop-blur-sm`}>
                <div className={`${getter != undefined ? 'pointer-events-auto' : 'pointer-events-none'} border border-transparent dark:border-gray-800 bg-white dark:bg-gray-900 sm:w-[85%] md:w-[60%] rounded-2xl p-4 max-h-[90vh] overflow-y-auto text-sm shadow-xl`}>
                    <div className="flex items-center justify-between">
                        <h3 className="text-sm font-bold text-gray-700 dark:text-gray-200">Assign Direct Permission To User</h3>
                            ...
                            <div className="grid grid-cols-2 gap-x-5 gap-y-1">
                                {
                                    permissions.map((item, index) => (
                                        <label htmlFor={`permissions${index}`} className="flex items-center space-x-1" key={index}>
                                            <input value={item.id} onChange={handleChecked} checked={data.permissions.includes(item.id + '')} type="checkbox" name="permissions[]" id={`permissions${index}`} />
