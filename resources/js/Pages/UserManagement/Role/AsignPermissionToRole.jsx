...
export default function AsignPermissionToRole(props) {
    ...
    function submit(e) {
        e.preventDefault()
        post(route('roles.asign_permission', role), {
            onSuccess: ()=>{
                setModalState(false)
            }
        })
                            ...
                            <div className="grid grid-cols-2 gap-x-5 gap-y-1">
                                {
                                    permissions.map((item, index) => (
                                        <label htmlFor={`permissions${index}`} className="flex items-center space-x-1" key={index}>
                                            <input value={item.id} onChange={handleChecked} defaultChecked={data.permissions.includes(item.id+'')} type="checkbox" name="permissions[]" id={`permissions${index}`} />
